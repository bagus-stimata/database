...
public class JenisKegiatanController {
    ...
    @RequestMapping("/master/kegiatanjenis")
    public String listIndex(final Model viewModel) {

        // final Authentication auth = SecurityContextHolder.getContext().getAuthentication();

  
        final TbKegiatanJenis newDomain =new TbKegiatanJenis();
        ...
        viewModel.addAttribute("allData", kegiatanJenisRepository.findAll());

        logger.info("# Form Task");
      
        return "master/kegiatanjenis/kegiatanjenis_list"; 
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = {"/master/kegiatanjenis_form/save_process"}, method = RequestMethod.POST)
    public String saveProcess(@ModelAttribute("domain") final TbKegiatanJenis domain, @ModelAttribute("domainDetil1") final TbKegiatanJenis domainDetil1, final RedirectAttributes redirectAttributes) {
        if (domain.getTempInt1()==0) {
            TbKegiatanJenis newDomain = new TbKegiatanJenis();
            ...
            if (kegiatanJenisRepository.save(newDomain) != null) {
            ...
            return "redirect:/master/kegiatanjenis/edit_form/" + newDomain.getId();

        }else if (domain.getTempInt1()==1) {
            TbKegiatanJenis domainUpdate = kegiatanJenisRepository.findById(domain.getId() );
            ...
            kegiatanJenisRepository.save(domainUpdate);

            redirectAttributes.addFlashAttribute("saveUser", "success");
            return "redirect:/master/kegiatanjenis/edit_form/" + domain.getId();
        }
       
        
        return "redirect:/master/kegiatanjenis/";
    }


    @PreAuthorize("hasAnyRole({'" + Role.ADMIN + "', '" + Role.ADMIN + "'})") //Perhatikan hasRole dan hasAnyRole
    @RequestMapping(value = "/master/kegiatanjenis/{operation}/{id}", method = RequestMethod.GET)
    public String toFormOperation(@PathVariable("operation") final String operation,
        ...
        logger.info("/master/kegiatanjenis/operation: {} ", operation);

       if (operation.equals("delete")) {
            try {
                TbKegiatanJenis domainToDelete = kegiatanJenisRepository.findById(id);        
                // hati-hati kecamatanRepository.deleteAll(domainToDelete.getListDesa() );
                kegiatanJenisRepository.delete(domainToDelete);               
            ...
            if (newDomain != null) {
                ...
                return "master/kegiatanjenis/kegiatanjenis_form";
            }

        } else if (operation.equals("edit_form")) {

            TbKegiatanJenis domain = kegiatanJenisRepository.findById(id);
            if (domain != null) {
                domain.setTempInt1(1); //0.New Form, 1.Edit Form, 3.Delete
                //Password tidak bisa di decode ya?


                // TbKegiatanJenisRoles domainDetil1 = new TbKegiatanJenisRoles();
                // TbKegiatanJenisRoles domainDetil2 = new TbKegiatanJenisRoles();
                // // for (TbKegiatanJenisRoles domainDetil: domain.getFuserRoles()) {
    ...
    @RequestMapping("/master/kegiatanjenis/perbaikan")
    @ResponseBody
